...
class App extends Component {
  ...
  componentDidMount = () => {
    this.timerId = setInterval(this.onStatusChange, 1000)
  }
  ...
  imageClick = thumbnailUrl => {
    ...
    const imageValue = imagesList.filter(
      eachList => eachList.thumbnailUrl === thumbnailUrl,
    )
  ...
  render() {
    const {timer, imgUrl, score, isTrue} = this.state
        ...
        {!isTrue && (
          <div className="total-section">
            <div className="top-img-container">
              <img src={imgUrl} alt="match" className="top-img" />
            </div>
            <div className="tabsList-container">
              <ul className="unorder-tabsList-container">
                {tabsList.map(eachTab => (
                  <li className="list-tabsList-container" key={eachTab.tabId}>
                    <button
                      type="button"
                      className="button"
                      onClick={() => this.onUpdateTabId(eachTab.tabId)}
                    >
                      {eachTab.displayText}
                    </button>
                  </li>
            ...
            <div className="bottom-section-container">
              <ul className="unorder-section-container">
                {filteredList.map(eachList => (
                  <li className="list-thumbnail" key={eachList.id}>
                    <button
                      type="button"
                      className="button"
                      onClick={() => this.imageClick(eachList.thumbnailUrl)}
                    >
                      <img
